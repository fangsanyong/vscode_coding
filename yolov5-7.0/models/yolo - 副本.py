...
class Detect(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            cls_channel = 5 + self.nc
            seg_channel = int(out_channels/self.na) - 5 - self.nc
            self.conv_cls = nn.Conv2d(self.m[i].in_channels, cls_channel*self.na, 1)
            self.conv_seg = nn.Conv2d(self.m[i].in_channels, seg_channel*self.na, 1)
            per_na = cls_channel+seg_channel
            
            src_conv=self.m[i]
            x_input=x[i]
            for i in range(self.na):
                 self.conv_cls.weight.data[cls_channel*i:cls_channel*(i+1),:,:,:] = src_conv.weight.data[per_na*i:per_na*i+cls_channel,:,:,:]
                 self.conv_cls.bias.data[cls_channel*i:cls_channel*(i+1)] = src_conv.bias.data[per_na*i:per_na*i+cls_channel]
                 self.conv_seg.weight.data[seg_channel*i:seg_channel*(i+1),:,:,:] = src_conv.weight.data[per_na*(i) + cls_channel:per_na*(i+1),:,:,:]
                 self.conv_seg.bias.data[seg_channel*i:seg_channel*(i+1)] = src_conv.bias.data[per_na*(i) + cls_channel:per_na*(i+1)]
    
            c,s=self.conv_cls(x_input).sigmoid(), self.conv_seg(x_input)
            z.append(c)
            z.append(s)
        return z
        #    #continue
